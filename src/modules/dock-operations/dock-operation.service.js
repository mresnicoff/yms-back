...
const assignDock = async ({
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const appointment = checkIn.appointment;
    ...
    const dock = await tx.dock.findFirst({
      where: {
        groupId: appointment.dockGroupId,
...
const getQueue = async (dockGroupId) => {
  ...
  queue.sort(
    (a, b) =>
      new Date(a.checkIn.arrivalTime) -
      new Date(b.checkIn.arrivalTime)
  );

  return queue.map((appointment, index) => ({
    position: index + 1,
    appointmentId: appointment.id,
    supplier: appointment.supplier.name,
    operationType: appointment.operationType,
    arrivalTime: appointment.checkIn.arrivalTime
  }));
};

const finishDockOperation = async ({
  dockOperationId
}) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const dockOperation =
      await tx.dockOperation.findUnique({
        where: {
          id: dockOperationId
        },
        include: {
    ...
    if (dockOperation.status === "FINISHED") {
      throw new Error(
        "Dock operation already finished"
      );
    }

    const dock = dockOperation.dock;

    const completedAppointment =
      dockOperation.checkIn.appointment;

    await tx.dockOperation.update({
      where: {
        id: dockOperationId
      },
      data: {
    ...
    const newDockOperation =
      await tx.dockOperation.create({
        data: {
          dockId: dock.id,
          checkInId:
            nextAppointment.checkIn.id,
          assignedById:
            dockOperation.assignedById,
          startedAt: new Date(),
          status: "ASSIGNED"
        }
      });
